...
class yolox_ros(yolox_py):
    ...
    def imageflow_callback(self,msg:Image) -> None:
        try:
            ...
            boxes_xyxy /= ratio
            dets = multiclass_nms(boxes_xyxy, scores, nms_thr=0.75, score_thr=0.9)
            ...
            if dets is not None:
                ...
                painting = COCO_CLASSES[int(final_cls_inds[0])]
                if painting not in self.images_detected:
                    self.images_detected.append(painting)
            
            self.get_logger().info(str(self.images_detected))
